...
def select_and_publish_goal():
    ...
    # NUEVO CRITERIO DE PARADA: Fronteras inalcanzables
    if mejor_objetivo_x is None:
        rospy.logwarn("Todas las fronteras restantes son inalcanzables (Lista Negra).")
        rospy.loginfo("¡Asumiendo entorno mapeado al máximo posible! Iniciando parada...")
           
        rospy.signal_shutdown("Criterio de parada alcanzado: Sin fronteras accesibles.")
        return
...
if __name__ == '__main__':
    try:
        ...
        start_time = time.time()  # Iniciar el temporizador al comenzar la exploración
